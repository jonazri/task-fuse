...
- [ ] 1.2 Implement core data types in `internal/store/types.go`
  - Implement `TaskStatus` type with constants: `StatusPending`, `StatusDoing`, `StatusDone`, `StatusFailed`
...
- [ ] 1.3 Implement TaskStore with concurrent access support in `internal/store/store.go`
...
- [ ] 2.1 Implement checkbox parsing in `internal/parser/parser.go`
...
- [ ] 5.1 Implement parent status derivation in `internal/store/status.go`
  ...
  - Follow precedence: doing > failed > pending > done
...
- [ ] 6.2 Implement status directories
  - Create `pending/`, `doing/`, `done/`, `failed/` directories at mount root
...
- [ ] 9.3 Implement status transition validation
  - Allow: pending→doing, doing→done/pending/failed, done→doing, failed→pending/doing
