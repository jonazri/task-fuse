...
func TestFile_Attr_IndexFile(t *testing.T) {
	...
	// Index files have size 0 for now (will be implemented in Task 7)
	if attr.Size != 0 {
		t.Errorf("Size = %d, want 0 for index files", attr.Size)
	}
}
...
func TestFile_Read_IndexFile(t *testing.T) {
	...
	// Index files return empty content for now (will be implemented in Task 7)
	if len(resp.Data) != 0 {
		t.Errorf("Read() returned %d bytes, want 0 for index files", len(resp.Data))
	}
}
